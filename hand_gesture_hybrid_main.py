...
class AdaptiveFrameQualityAnalyzer:
    ...
    def analyze(self, frame: np.ndarray) -> dict:
        ...
        e_score = np.clip(edge_density * 900.0, 0.0, 100.0)
...
class CommandEngine:
    ...
    def update(self, pose_text, pose_score, motion_text, motion_score,
        ...
        if pose_score < 0.20 or motion_score < 0.20:
            return None
...
def draw_gesture_info(image, brect, hand_sign_text, motion_text, motion_source="", command_text=""):
    ...
    if command_text != "":
        cv2.putText(image, "Command:" + command_text, (10, 105),
...
def main():
    ...
    with open('model/hand_center_history_classifier/hand_center_history_classifier_label.csv', encoding='utf-8-sig') as f:
    ...
    command_rules = [
        CommandRule("Open", "Swipe Right", "OPEN_APP", cooldown_ms=1200),
        CommandRule("Open", "Swipe Left", "CLOSE_APP", cooldown_ms=1200),
        CommandRule("Open", "Swipe Up", "SCROLL_UP", cooldown_ms=900),
        CommandRule("Open", "Swipe Down", "SCROLL_DOWN", cooldown_ms=900),
        CommandRule("Point", "Circle", "CONFIRM", cooldown_ms=1200),
        CommandRule("Point", "Clockwise", "NEXT", cooldown_ms=1200),
        CommandRule("Point", "Counter Clockwise", "PREVIOUS", cooldown_ms=1200),
        CommandRule("Fist", "Swipe Down", "CANCEL", cooldown_ms=1200),
        CommandRule("Fist", "Swipe Up", "UNDO", cooldown_ms=1200),
    ]
        ...
        if detection_result.hand_landmarks:
            for hand_landmarks in detection_result.hand_landmarks:
                ...
                # Motion source B: hand-center history for non-pointer gestures
                    ...
                    center_point = get_hand_center_from_brect(brect)
