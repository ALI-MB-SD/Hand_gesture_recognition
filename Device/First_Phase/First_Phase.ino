...
const char* MQTT_USER = "esp32_light_2";
...
// -------------------- TOPICS --------------------
String getCmdTopic() {
    return "gesture/device/" + String(DEVICE_ID) + "/cmd";
}
String getAckTopic() {
    return "gesture/device/" + String(DEVICE_ID) + "/ack";
}
String getStatusTopic() {
    return "gesture/device/" + String(DEVICE_ID) + "/status";
}
String getHeartbeatTopic() {
    return "gesture/device/" + String(DEVICE_ID) + "/heartbeat";
}
...
void publishHeartbeat() {
    JsonDocument doc;
    doc["device_id"] = DEVICE_ID;
    doc["source"] = "heartbeat";
    doc["uptime_seconds"] = (uint32_t)(millis() / 1000);

    int rssi = WiFi.RSSI();
    doc["wifi_rssi"] = rssi;
    doc["wifi_quality"] = getWifiQuality(rssi);
...
void connectMQTT(){
    while (!mqttClient.connected()){
        ...
        if( mqttClient.connect(clientId.c_str(), MQTT_USER, MQTT_PASSWORD, statusTopic.c_str(), 1, true, willPayload.c_str()) ){
            
            Serial.println("Connected");
            mqttClient.subscribe(getCmdTopic().c_str(), 1);

            Serial.print("Subscribed: ");
            Serial.println(getCmdTopic());
